...
function RenderTargetInspector(a, b, c) {
    function e(a, b, c) {
        ...
        for (var d = b.next(); !d.done; d = b.next()) a.addEventListener(d.value, c)
    }
    ...
    function f(a) {
        var b = d.viewerCanvas, c = b.width, b = b.height;
        a.project(d.viewerCamera);
        a.x = c / 2 * a.x + c / 2;
        a.y = b / 2 * a.y + b / 2
    }
    ...
    b = void 0 === b ? 256 : b;
    c = void 0 === c ? "left" : c;
    if (this.__proto__.constructor !== RenderTargetInspector) return RenderTargetInspector.instance = new RenderTargetInspector(a,
        b, c), RenderTargetInspector.instance;
    var d = this, k = document.createElement("div");
    this.size = b;
    this.renderer = a;
    this.views = [];
    ...
    this.material = new THREE.ShaderMaterial({
        uniforms: {map: {value: null}, size: {value: new THREE.Vector2}},
        defines: {MAP_TYPE: 0},
    ...
    var l = new THREE.Vector2, n = new THREE.Vector2, m = new THREE.Vector2, p = new THREE.Vector2,
        r = new THREE.Vector3, q = new THREE.Vector3, v = new THREE.Plane, x = new THREE.Vector3,
        t = new THREE.Raycaster, u = new THREE.Vector3, w =
            new THREE.Vector3;
    v.normal.set(0, 0, 1);
    e(this.viewerCanvas, "mouseup touchend mousedown touchstart mousemove touchmove DOMMouseScroll mousewheel", function () {
        function a() {
            ...
            f(u);
            f(w);
            var a = d.viewerBar.style, b = d.viewerCanvas.height;
        ...
        function b() {
            ...
            0 <= q.x && q.x < c.width && 0 <= q.y && q.y < c.height && (d.getSample(c, q.x, q.y, h), d.viewerSample.innerHTML = q.x + 1 + " : " + (q.y + 1) + "<br/>r: " + h[0] + "<br/>g: " + h[1] + "<br/>b: " + h[2] + "<br/>a: " + h[3])
        }

        var c, e = !1, k = !1, h = [0, 0, 0, 0];
        return function (f) {
            c = d.currentView;
            var g = d.viewerCanvas, h = g.width, g = g.height;
            ...
            switch (f.type) {
                ...
                case "touchmove":
                    e ?
                        (l.copy(r).sub(n), d.offset.sub(l), d.viewerCamera.position.set(d.offset.x, d.offset.y, 0), d.viewerCamera.updateMatrixWorld(!0), t.setFromCamera(m, d.viewerCamera), t.ray.intersectPlane(v, r), n.copy(r), a()) : k && b();
                    break;
                case "mousedown":
                case "touchstart":
                    "renderTargetHelperViewer" === f.target.id ? d.show(null) : f.which === d.which.left ? (k = !0, b()) : (d.viewerCanvas.cursor = "grab", e = !0, n.copy(r));
                    break;
                case "mouseup":
                case "touchend":
                    k = !1, e && (e = !1, d.viewerCanvas.cursor = "default")
            }
        }
    }());
...
var $jscomp$compprop1 = {}, $jscomp$compprop2 = {};
RenderTargetInspector.prototype = {
    constructor: RenderTargetInspector,
    _vec2: new THREE.Vector2,
    decoding: {"default": 0, depth: 1},
    which: {none: 0, left: 1, middle: 2, right: 3},
    strides: ($jscomp$compprop1[THREE.AlphaFormat] = 1, $jscomp$compprop1[THREE.RedFormat] = 1, $jscomp$compprop1[THREE.RGFormat] = 2, $jscomp$compprop1[THREE.RGBFormat] = 3, $jscomp$compprop1[THREE.RGBAFormat] = 4, $jscomp$compprop1),
    types: ($jscomp$compprop2[THREE.UnsignedByteType] = Uint8Array, $jscomp$compprop2[THREE.FloatType] = Float32Array, $jscomp$compprop2),
    ...
    show: function (a) {
        if (this.currentView = a) if (this.viewerLabel.textContent =
            a.name + " (" + a.width + "x" + a.height + ")", !this.popup) {
            var b = this, c = this.viewerCanvas;
            ...
            this.listen(h, "resize contextmenu beforeunload", function (a) {
                switch (a.type) {
                    ...
                    case "resize":
                        c.width = h.innerWidth - 10, c.height = h.innerHeight - 10, b.updateCamera(), b.update(!1)
                }
            });
    ...
    add: function (a, b, c) {
        ...
        if (a instanceof Array) {
            ...
            for (var e = a.next(); !e.done; e = a.next()) this.add(e.value, b, c)
        } else {
            b = b || a.name;
            var e = this.createElement('\n\t\t\t\t<div>\n\t\t\t\t\t<div style="display:flex;">\n\t\t\t\t\t\t<div style="padding:2px;"></div>\n\t\t\t\t\t\t<div style="display:flex; flex-grow:1; justify-content:flex-end;">\n\t\t\t\t\t\t\t<div uid="toImage" title="To image" style="padding:2px">&#9715</div>\n\t\t\t\t\t\t</div>\n\t\t\t\t\t</div>\n\t\t\t\t\t<canvas></canvas>\n\t\t\t\t</div>\n\t\t\t\t'),
                h = "renderView" +
                    this.views.length, f = e.children[1], d = f.getContext("2d"), k = f.style, g = this.size;
            d.font = .1 * g + "px Arial";
            ...
            k = this.material;
            void 0 !== c && (k = k.clone(), k.needsUpdate = !0, void 0 !== c.type && (k.defines.MAP_TYPE = this.decoding[c.type]), c.onBeforeCompile && (k.onBeforeCompile = c.onBeforeCompile, k.needsUpdate =
                !0));
            this.views.push({id: h, name: b, material: k, div: e, ctx: d, canvas: f, target: a, options: c})
        }
    },
    remove: function (a) {
    ...
    getByTarget: function (a) {
        for (var b = 0, c = this.views.length; b < c; b++) {
            var e = this.views[b];
            if (e.target === a) return e
        }
    },
    getByID: function (a) {
        for (var b = $jscomp.makeIterator(this.views),
                 c = b.next(); !c.done; c = b.next()) if (c = c.value, c.id === a) return c
    },
    getTexture: function (a) {
        if (a.isWebGLRenderTarget) return a.texture;
        if (a.isTexture) return a;
    ...
    getSample: function () {
        ...
        return function (c, e, h, f) {
            ...
            if (d.isWebGLRenderTarget) c.buffer || (c.buffer = this.allocateBuffer(d.texture, d.width, d.height)), l = c.buffer, g = this.strides[d.texture.format], this.renderer.readRenderTargetPixels(d, e, h, 1, 1, l); else {
                g = void 0 !== d.value ? d.value : d;
                if (!g || !g.isDataTexture) return !1;
                k = (e + h * g.image.width) * this.strides[g.format];
                l = g.image.data;
                g = this.strides[g.format]
            }
            for (e = 0; e < g; e++) f[e] = l[k + e];
            "depth" === c.options.type && (b.fromArray(f), f[0] = b.dot(a), f[1] = f[2] = f[3] = 0);
            return !0
        }
    }(),
    open: function (a) {
    ...
    toBlob: function (a, b) {
        var c = this.renderer, e = this._vec2, h = e.x, f = e.y;
        c.getSize(e);
        var d = c.autoClear;
        c.autoClear = !1;
        c.setRenderTarget(null);
        var k = a.ctx, g = a.material, l = this.getTarget(a), n = l ? this.getTexture(l) : null, m = 0, p = 0;
        l.isWebGLRenderTarget ? (m = l.width, p = l.height) :
            n.image && n.image.width && n.image.height && (m = n.image.width, p = n.image.height);
        l = !1;
        if (m > h || p > f) h = Math.max(m, h), f = Math.max(p, f), l = !0, c.setSize(h, f);
    ...
    update: function (a) {
        var b = this.renderer, c = this._vec2;
        b.getSize(c);
        var e = b.autoClear, h = b.getRenderTarget();
        b.autoClear = !1;
        ...
        if (!1 !== a) {
            ...
            for (var f = this.views.length; a < f; a++) {
                var d = this.views[a], k = d, g = k.ctx, k = k.material, l = this.getTarget(d),
                    n = l ? this.getTexture(l) : null;
                n ? (l.isWebGLRenderTarget ? (d.width = l.width, d.height = l.height, d.ready = !0) : n.image && n.image.width && n.image.height ?
                    (d.width = n.image.width, d.height = n.image.height, d.ready = !0) : d.ready = !1, this.quad.material = k, k.uniforms.map.value = n, k.uniforms.size.value.set(d.width, d.height), b.render(this.scene, this.camera), g.clearRect(0, 0, this.size, this.size), g.drawImage(b.domElement, 0, c.y - this.size, this.size, this.size, 0, 0, this.size, this.size)) : (d.ready = !1, g.clearRect(0, 0, this.size, this.size), g.fillStyle = "gray", g.fillRect(0, 0, this.size, this.size), g.fillStyle = "red", g.fillText("N/A", 10, 10))
            }
        }
        this.currentView && (a = this.viewerCamera,
            f = this.currentView, g = this.viewerCanvas, d = g.width, g = g.height, k = (k = this.getTarget(f)) ? this.getTexture(k) : null, b.setViewport(0, 0, d, g), b.setScissor(0, 0, d, g), l = Math.min(d, g), this.viewerQuad.scale.set(l, l, 1), this.viewerQuad.material = f.material, f.material.uniforms.map.value = k, b.setRenderTarget(null), b.clear(), b.render(this.viewerScene, a), this.ctx.clearRect(0, 0, d, g), this.ctx.drawImage(b.domElement, 0, c.y - g, d, g, 0, 0, d, g));
...
THREE.RenderTargetInspector = RenderTargetInspector;